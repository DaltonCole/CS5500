...
n_identlst      : /* epsilon */ {self.tokInfo("N_IDENTLST", "epsilon")}
...
n_inputlst      : /* epsilon */ {self.tokInfo("N_INPUTLST", "epsilon")}
...
n_intconst      : {True if $T_INTCONST is None or $T_INTCONST.text <= 0x7FFFFFFF else False}? n_sign T_INTCONST {print("ASDFGHJKL")} 
				| n_sign INVALINT {print("QWERTYUIO")} 
				;
...
n_multoplst     : /* epsilon */ {self.tokInfo("N_MULTOPLST", "epsilon")}
...
n_outputlst     : /* epsilon */ {self.tokInfo("N_OUTPUTLST", "epsilon")}
...
n_procdecpart   : /* epsilon */ {self.tokInfo("N_PROCDECPART", "epsilon")}
...
n_sign          : /* epsilon */ {self.tokInfo("N_SIGN", "epsilon")}
...
n_stmtlst       : /* epsilon */ {self.tokInfo("N_STMTLST", "epsilon")}
...
n_vardeclst     : /* epsilon */ {self.tokInfo("N_VARDECLST", "epsilon")}
                | n_vardec T_SCOLON n_vardeclst {self.tokInfo("N_VARDECLST", "N_VARDEC", "T_SCOLON", $T_SCOLON.text, "N_VARDECLST")} ;

n_vardecpart    : /* epsilon */ {self.tokInfo("N_VARDECPART", "epsilon")}
...
T_NOT 			: 'not' ;
T_PROC 			: 'procedure' ;
T_PROG 			: 'program' {print("PROGRAM")};
T_READ 			: 'read' ;
T_THEN 			: 'then' ;
T_VAR 			: 'var' ;
T_WHILE 		: 'while' ;

T_INTCONST 		: VALIDINT {print("BAMMO!")};
...
SIGN			: [+-] ;
COUNT_DIGIT		: [1-9] ; 
DIGIT			: ([0] | COUNT_DIGIT) ;/* {print(getText())} */ 
...
VALIDINT		: ([0*] COUNT_DIGIT DIGIT{0,8}) {print("VALIDINT")} | ([0*]) {print("VALIDINT")} ;
MAYBEINT		: ([0*] COUNT_DIGIT DIGIT{9,9}) {print("MAYBE")} ;
INVALINT		: ([0*] COUNT_DIGIT DIGIT*) ;

IDENT			: (UNDERSCORE | ALPHA)(ALPHANUM | UNDERSCORE)* {print("IDENT")};
